/* Synapse Web UI — shared primitives.
   Lucide-style outline icons (stroke 1.75) + brand primitives.
...
function Button({ variant = 'primary', children, arrow, icon, as = 'button', href, onClick, type, disabled, style }) {
  const cls = `btn btn-${variant}`;
  const inner = <>{icon && <Icon name={icon} size={15} stroke={2} />}{children}{arrow && <Icon name="arrow-right" size={14} stroke={2} />}</>;
...
function Chip({ s, label }) {
  const text = label || { running: 'running', passed: 'passed', blocked: 'blocked', idle: 'queued',
    online: 'online', offline: 'offline', ready: 'ready', installing: 'installing', failed: 'failed',
    pending: 'pending', recovering: 'recovering' }[s] || s;
  return <span className={'status-chip ' + s}>{text}</span>;
}

Object.assign(window, { Icon, LogoMark, Kicker, Button, HatchCorners, Chip, ICON_PATHS });
